...
export const SaveEdit = ({imgSrc,setImgSrc,file,setFile,setAllertmsg,setOpen}) => {
  ...
  const uploadImage = () => {  
      ...
      .then( response => {
        ...
        dispatch({type:"alertMessage",payload:'Image uploaded succesfully'})
        setImgSrc(null)     
    })
    .catch(error => {
  ...
  <> {!openCrop?
    <Box sx={{
      ...
      <Grid container rowSpacing={1} columnSpacing={1}>
            ...
            <Grid item sm={12} md={4} >
               <Stack  spacing={3}  justifyContent="center"
                        alignItems="flex-start"  direction={{xs:'row',md:'column'}} sx={{marginTop:'15px'}}> 
                    <Button  variant="contained" onClick={()=>uploadImage()}>Save</Button>
                    <Button variant="contained" onClick={()=>setOpenCrop(true)} >Crop</Button>
                    <Button variant="contained" onClick={()=>toHome()}>Cancel</Button>
     ...
     <CropEasy {...{ imgSrc:CropImg, setOpenCrop, setImgSrc, setFile }}></CropEasy>
     }
    </>
  )
}
